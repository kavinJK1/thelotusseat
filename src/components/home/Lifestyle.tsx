...
export default function Lifestyle() {
  return (
    <section className="relative bg-paper border-y border-line overflow-hidden">
      <div aria-hidden className="absolute inset-0 tech-grid opacity-60" />
      {/* single, purposeful glow — the one place ambience earns its keep. On paper it
          reads as a cork wash rather than a light source, so it stays well under 10%. */}
      <div
        aria-hidden
        className="absolute -right-32 top-1/2 -translate-y-1/2 w-[520px] h-[520px] rounded-full"
        style={{ background: 'radial-gradient(circle, oklch(0.63 0.108 52 / 0.09), transparent 70%)' }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 py-24 md:py-32">
        <FadeIn>
          ...
          <Link
            href="/checkout"
            className="mt-10 inline-flex items-center gap-2 bg-ink text-paper px-6 py-3.5 rounded-[3px] text-sm font-medium hover:bg-graphite-soft transition-colors"
          >
            Pre-Order Now
